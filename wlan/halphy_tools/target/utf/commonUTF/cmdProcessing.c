...
void setRates4Report (TCMD_CONT_RX *contRx, A_UINT32 *pShortGuard, A_UINT32 *pShortGuard11AC,
					A_UINT32 *pRateMask0, A_UINT32 *pRateMask1, A_UINT32 *rate11ACMask, A_UINT32 *pRate11AC4x4Mask, A_UINT32 *pRateMask160)
{
	...
	//11AC 4x4.. rates
	for(i=(TCMD_11AC3x3_MAX_RATES + 3); i<RATES_11AC_160;i++)
	{
		rateBitIndex = Rate2MaskBitNo[i];
	...
	//160MHZ
	for(i=RATES_11AC_160;i<TCMD_MAX_RATES;i++)
	{
		rateBitIndex = Rate2MaskBitNo[i];
	...
	// A_PRINTF("11AC rateBitIndex %d rateMask %d cnt %d\n",rateBitIndex,i,rateCnt);
		if (rateCnt) {
			//A_PRINTF("11AC rateBitIndex %d rateMask %d cnt %d\n",rateBitIndex,i,rateCnt);
			value = 1 << mask;
			*pRateMask160 = *pRateMask160 | value;
		}
		else
		{
			rateCnt = contRx->u.report.rateCntShortGuard[i];
...
void sendRxReportResponse(wlan_tcmd_dev_t *dev,TCMD_CONT_RX *contRx)
{
    ...
    if ( ret == TRUE )
    {
		...
		memset(rxReportCnt,0,(245 * sizeof(A_UINT16)));
          
       // setRates4Report(contRx, shortGuard, shortGuard11AC, &rateMask0, &rateMask1, rate11ACMask, &rate11AC4x4Mask);
		setRates4Report(contRx, shortGuard, shortGuard11AC, &rateMask0, &rateMask1, rate11ACMask, &rate11AC4x4Mask,&rateMask160);
	...
	//A_PRINTF_ALWAYS("rcMaskac4x4 =%x\n", rate11AC4x4Mask);
		...
		addParameterToCommand((A_UINT8*)"rcMask160",(A_UINT8*)&rateMask160);
		
        j = 224;
        for(i=0;i<20;i++) {
            memset(rateCntBuf,0,sizeof(rateCntBuf));
            A_STRNCPY((char*)rateCntBuf,"rtCnt160", sizeof(rateCntBuf));
            getConcatBuf((char*)rateCntBuf,i, A_STRLEN(rateCntBuf), sizeof(rateCntBuf));

            value = rxReportCnt[j+1] << 16 | rxReportCnt[j];
#if defined(DBGLOG_FTM_SUPPORT)
			//DBGLOG_RECORD_LOG(WLAN_MODULE_FTM, -1, FTM_DBGID_SETUP_CHANNEL, DBGLOG_UTF, 5, 0xA2A2, j, i, rxReportCnt[j], rxReportCnt[j+1]);
			//DBGLOG_RECORD_LOG(WLAN_MODULE_FTM, -1, FTM_DBGID_SETUP_CHANNEL, DBGLOG_UTF, 5, 0xA3A3, value, rateCntBuf);
#endif
