...
void parseTLVSubmitReport()
{
    ...
    A_UINT32 rcMaskac4x4=0, rcMask160=0;
	...
	ret = getParams((A_UINT8*)"rcMask160",&data,&len);

	if ( ret == TRUE )
	{
			memcpy(&rcMask160,data,len);
			printf("rcMask 160 %d\n",rcMask160);
	}
    ...
    for (k=0;k<1;k++)
    {
        ...
        while (rateMask)
        {
            if (bit0 & rateMask)
            {
                ...
                index = (k*32+i)/2;
                //printf(" mcs bitmask i val= %d\n",i);

                memset(rateCntBuf,0,sizeof(rateCntBuf));
                snprintf((char*)rateCntBuf,sizeof(rateCntBuf),"%s%d","rtCnt160",index);
                ret = getParams((A_UINT8*)rateCntBuf,&data,&len);
                        //printf("DEBUG 11ac160 retval=%d, RateCntBitmask %d dataRate %d  index =%d rateCntBuf=%s\n", ret, i, dataRate, index, rateCntBuf);
...
void handleRXSTATUSRSP (void *parms)
{
	...
	printf("evm = %d, %d, %d, %d\n", pParms->evm[0], pParms->evm[1], pParms->evm[2], pParms->evm[3]);
