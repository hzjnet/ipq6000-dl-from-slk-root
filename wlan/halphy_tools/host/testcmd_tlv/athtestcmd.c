        ...
        case TCMD_LOOP:
	        ...
	        case TCMD_LM_TXINIT:       //
                   // tlv2Construct = TRUE;
                    ...
                    cmdStreamLen = (sizeof(TESTFLOW_CMD_STREAM_HEADER_V2) + pCmdStream->cmdStreamHeader.length);
                    rCmdStream = (A_UINT8 *)pCmdStream;
                    printf("cmdStreamLen=%d, pCmdStream->cmdStreamHeader.length=%d, sizeof(TESTFLOW_CMD_STREAM_HEADER_V2)=%zu ", cmdStreamLen, pCmdStream->cmdStreamHeader.length, sizeof(TESTFLOW_CMD_STREAM_HEADER_V2));
...
static A_BOOL readBinFile(char *fileName, void *buf, A_UINT32 *length, A_UINT32 length2Read)
{
    ...
    if (len2Read == (numBytes = fread((A_UCHAR *)buf, 1, len2Read, fp))) {
        ...
        if (feof(fp)) {
            printf("Read %zu from %s, expected length %d, check if it ok\n", numBytes, fileName, length2Read);
            rc = TRUE;
        }
        else if (ferror(fp)) {
