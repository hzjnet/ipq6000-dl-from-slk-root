...
#define WLAN_HALPHY_DBG_STATS_IS_CAL_FAIL_CNT_VALID_S      5
#define WLAN_HALPHY_DBG_STATS_IS_CAL_FAIL_CNT_VALID_M      0x00000020

#define WLAN_HALPHY_DBG_STATS_CAL_FAIL_CNT_S               0
#define WLAN_HALPHY_DBG_STATS_CAL_FAIL_CNT_M               0x0000ffff
...
#define WLAN_HALPHY_DBG_STATS_IS_CAL_FAIL_CNT_VALID_GET(dword) \
    (((dword) & WLAN_HALPHY_DBG_STATS_IS_CAL_FAIL_CNT_VALID_M) >> \
    WLAN_HALPHY_DBG_STATS_IS_CAL_FAIL_CNT_VALID_S)
#define WLAN_HALPHY_DBG_STATS_IS_CAL_FAIL_CNT_VALID_SET(dword, value) \
    do { \
        (dword) &= ~WLAN_HALPHY_DBG_STATS_IS_CAL_FAIL_CNT_VALID_M; \
        (dword) |= (((value) << WLAN_HALPHY_DBG_STATS_IS_CAL_FAIL_CNT_VALID_S) & \
            WLAN_HALPHY_DBG_STATS_IS_CAL_FAIL_CNT_VALID_M); \
    } while (0)

#define WLAN_HALPHY_DBG_STATS_CAL_FAIL_CNT_GET(dword) \
    (((dword) & WLAN_HALPHY_DBG_STATS_CAL_FAIL_CNT_M) >> \
    WLAN_HALPHY_DBG_STATS_CAL_FAIL_CNT_S)
#define WLAN_HALPHY_DBG_STATS_CAL_FAIL_CNT_SET(dword, value) \
    do { \
        (dword) &= ~WLAN_HALPHY_DBG_STATS_CAL_FAIL_CNT_M; \
        (dword) |= (((value) << WLAN_HALPHY_DBG_STATS_CAL_FAIL_CNT_S) & \
            WLAN_HALPHY_DBG_STATS_CAL_FAIL_CNT_M); \
    } while (0)
...
typedef struct {
     ...
     * bit 5: Represents if calibration failure counts are valid in the stats.
    ...
    /* calFailCnt bits 0:15
     *    Stats to count number of calibration failures including
     *    Home, Scan and FCS altogether.
     *    This calibration failure count is only valid if the
     *    IS_CAL_FAIL_CNT_VALID flag is set.
